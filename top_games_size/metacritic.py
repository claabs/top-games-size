...
def get_top_rated_exclusives(platform, **kwargs):
    limit = platform.limit
    min_rating = kwargs.get("min_rating")
    ...
    while len(all_games) < limit:
        ...
        # Send GET request to the API endpoint
        response = requests.get(
            f"https://internal-prod.apigee.fandom.net/v1/xapi/reviews/metacritic/critic/publications/{platform.exclusive_key}/web",
            params=params,
        )
    ...
    valid_games = list(
        filter(
            lambda x: x.get("score") > min_rating,
            all_games[:limit],
        )
    )
    valid_game_titles = list(
        map(lambda x: x.get("reviewedProduct").get("title"), valid_games)
    )
